...
* Dimensión 1: Consumo neto de electricidad per-cápita, oferta de energía secundaria per-cápita, consumo residencial de energía, el Producto Bruto Interno per-cápita, consumo energético final, y el Producto Bruto Interno per-cápita. \
...
El tercer componente, al igual que en 1984, representa la *dimensión ambiental*, mientras que el primero representará la *dimensión social* (abastecimiento de energía a distintos sectores de la sociedad, nivel de ingresos, etc.), y el segundo la *dimensión económica*. Estas últimas dos dimensiones están altamente vinculadas, y comparten correlaciones elevadas para variables comunes. >>ver por qué se da esto
...
```{r, fig.cap= "Dimensiones 1 y 2."}
plot.PCA(res.pca1996,choix='var',habillage = 'contrib',title="Círculo de correlaciones")
```


```{r,fig.cap= "Dimensiones 3 y 4."}
plot.PCA(res.pca1996,choix = "var",axes=c(3,4),habillage='cos2', title = "Círculo de correlaciones")
```
...
```{r, fig.cap="Dimensiones 1 y 2."}
plot.PCA(res.PCA1996,habillage='cos2')
```
