...
```{r correlaciones 1984, include=FALSE}
kable(cor(Energia1[Energia1$Año==1984,2:15])) %>% 
  kable_styling(full_width = F, fixed_thead = T)
```
...
```{r criterio BS 1984, fig.cap="Criterio Broken-sticks para la elección de componentes",cache=TRUE,out.width="80%"}
...
* Dimensión 1: Oferta de energía secundaria per-cápita, consumo neto de electricidad per-cápita, consumo residencial de energía, consumo energético final per-cápita, Producto Bruto Interno per-cápita, consumo de energía del sector transporte, oferta de energía primaria per- cápita, consumo industrial de energía. \
...
```{r, fig.cap= "Dimensiones 1 y 2.",out.width="80%"}
plot.PCA(res.pca1984,choix='var',habillage = 'contrib',title="Círculo de correlaciones para 1984",select='cos2  0',unselect=0)
```


```{r,fig.cap= "Dimensiones 3 y 4.",out.width="80%"}
plot.PCA(res.pca1984,choix = "var",axes=c(3,4),habillage='cos2', title = "Círculo de correlaciones para 1984",select='cos2  0',unselect=0)
```
...
```{r, fig.cap="Dimensiones 1 y 2.", out.width="80%"}
plot.PCA(res.pca1984,title="Gráfico perspectiva de Países 1984",habillage = 'cos2')
```
...
```{r, fig.cap="Dimensiones 3 y 4.",out.width="80%"}
plot.PCA(res.pca1984,axes=c(3,4),title="Gráfico perspectiva de Países 1984",habillage = 'cos2')
```
...
- valores bajos de proporción de consumo de energía secundaria per-cápita, oferta de energía secundaria per cápita, proporción del consumo energético del sector transporte, PBI per-cápita, consumo eléctrico per capita, productividad energética, consumo energético final per-cápita, productividad industrial, y la oferta de energía primaria. \
...
- valores bajos de la proporción de consumo eneregético residencial. \
...
- valore bajos de la proporción de consumo eneregético residencial. \
...
```{r summary de ACP, va en anexos,include=FALSE}
summary(res.pca1984)
```
...
```{r summary de las variables originales va en anexos, include=FALSE}
(summary(Energia1[,2:15]))
```


## Situación regional en 1996


Matriz de correlaciones entre variables para el año 1996:


```{r correlaciones 1996}
kable(cor(Energia1[Energia1$Año==1996,2:15])) %>% 
  kable_styling(full_width = F, fixed_thead = T)
```


Para el año 1996, observamos que se mantienen las correlaciones altas (>0.70) detectadas para 1984. Se suman al listado la relación negativa entre la oferta secundaria de energía per-cápita y la proporción de consumo residencial.
...
```{r criterio BS 1996, fig.="Criterio Broken-sticks para la elección de componentes",cache=TRUE, out.width="80%"}
evplot(vp96[,1])
```
...
* Dimensión 1: Consumo neto de electricidad per-cápita, oferta de energía secundaria per-cápita, consumo residencial de energía, Producto Bruto Interno per-cápita, consumo energético final. \
* Dimensión 2: Productividad energética, productividad industrial, emisión de Co2 por unidad energética  de consumo, oferta de energía primaria per-cápita, proporción del consumo energético del sector transporte, proporción del consumo energético del sector industrial, proporción del consumo de energía secundaria. \
* Dimensión 3: Emisión de CO2 por unidad de consumo energético, emisión de CO2 per-cápita. \
* evaluar incluir la 4ta acá
...
El conjunto de dimensiones más relevante sigue siendo la social y la económica, incluso con un leve aumento de su pdoer explicativo en comparación con 1984. 

Visualizamos en los correspondientes círculos de correlaciones:


```{r, fig.cap= "Dimensiones 1 y 2.",out.width="80%"}
plot.PCA(res.pca1996,choix='var',habillage = 'contrib',title="Círculo de correlaciones para 1996",select='cos2  0.6',unselect=0)
```


```{r,fig.cap= "Dimensiones 3 y 4.",out.width="80%"}
plot.PCA(res.pca1996,choix = "var",axes=c(3,4),habillage='cos2', title = "Círculo de correlaciones para 1996",select='cos2  0.6',unselect=0)
```
...
```{r, fig.cap="Dimensiones 1 y 2.",out.width="80%"}
plot.PCA(res.pca1996,habillage = "cos2",title="Gráfico perspectiva de Países",select='cos2  0.5')
```
...
```{r, fig.cap="Dimensiones 3 y 4.",out.width="80%"}
plot.PCA(res.pca1996,axes=c(3,4),title="Gráfico perspectiva de Países",habillage = "cos2")
```
...
Grupo 1: 
- valores altos de proporción de consumo energético residencial.
- valores bajos de consumo eléctrico per capita, oferta de energía secundaria per cápita, PBI per-cápita, consumo energético final per-cápita, proporción de consumo de energía secundaria per-cápita, emisión de CO2 per-cápita, propoerción de consumo energético industrial, del sector transporte, y la productividad energética.
