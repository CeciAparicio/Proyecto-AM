...
#### Países y rango de años contenidos en los datos:

\ 
```{r}
kable(unique(pais), booktabs=TRUE)
kable(unique(anio), booktabs=TRUE) %>% 
...
\
```{r}
kable(Energia1[,2:15],booktabs=TRUE) %>% 
...
kable(cor(Energia1[2:15]),booktabs=TRUE) %>% 
  kable_styling(latex_options = "scale_down")
```
...
```{r summary de las variables originales va en anexos, results='asis'}
kable(summary(Energia1[,2:15]))
```
