...
#### Países que se incluyen en el análisis:
...
Para el año 1984, observamos que existe correlaciones altas (>0.70) en varios casos (ver tabla en Anexos). Se destaca principalmente la correlación entre la productividad energética (*Prod_Ene*) con el ingreso per cápita (*PBIpc*), el consumo energético per cápita (*ConsENpc*) con la oferta primaria de energía per cápita (*O_PRIpc*) y con el consumo eléctrico neto (*C_ELEpc*), y de esta última también con la oferta primaria de energía per cápita.

Para el año 1996, observamos que se mantienen las correlaciones elevadas detectadas para 1984 (ver tabla en Anexos). Se suman al listado la relación negativa entre la oferta secundaria de energía per-cápita y la proporción de consumo residencial.
...
kable(head(Energia1[2:9]),caption = "Primeras 5 filas de datos",booktabs=TRUE)


kable(head(Energia1[10:15]),caption = "Primeras 5 filas de datos (cont.)",booktabs=TRUE)
```
...
kable(head(Energia_st[10:17]),caption = "Primeras 5 filas de datos estandarizados (cont.)",booktabs=TRUE)
```
...
kable(cor(Energia1[Energia1$Año==1984,2:9]), caption="Matriz de correlaciones. 1984.",booktabs=TRUE)


kable(cor(Energia1[Energia1$Año==1984,10:15]), caption="Matriz de correlaciones. 1984. (cont.)",booktabs=TRUE)
```
